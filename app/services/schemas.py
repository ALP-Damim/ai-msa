...
class EvaluateBatchRequest(BaseModel):
	...
	def lengths_match(self):
		lengths = {len(self.questions), len(self.answers), len(self.studentAnswers)}
		if len(lengths) != 1:
			raise ValueError("questions, answers, studentAnswers must have the same length")
		if self.count is not None and self.count != len(self.questions):
			raise ValueError("count must equal the length of questions/answers/studentAnswers")
		return self
